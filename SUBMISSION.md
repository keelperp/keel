...
我们**不请求豁免,也不声称这比 3% 温和**。以下是完整数据,包括对我们不利的部分。

### 口径不同

| | Flap commission 推荐 | 本金库 |
|---|---|---|
| 抽取对象 | 每笔交易的**税** | 仓位的**收益** |
| 2% 税下的参考值 | ~3% | — |
| 项目方从税里拿 | — | **0%** |
...
金库收到的税 **100% 进入仓位**。项目方在税这一层拿零。

### 折算成"等价抽税率"是多少

以链上实测参数计算(Venus vUSDT 借款 2.62% / vBNB 存款 0.13%,3 倍结构的持仓成本
**−4.87%/年**),把项目方一年的收入除以同期税收:

| BNB 年涨幅 | 仓位收益 | 项目方等价抽成 | 对比 ~3% |
|---:|---:|---:|:--|
| −20% | −64.9% | **0%** | 低 |
| −10% | −34.9% | **0%** | 低 |
| 0% | −4.9% | **0%** | 低 |
| **+4.1%** | +7.5% | **3.0%** | **持平** |
| +10% | +25.1% | **10.1%** | 高 3.4 倍 |
| +20% | +55.1% | **22.1%** | 高 7.4 倍 |
| +50% | +145.1% | **58.1%** | 高 19 倍 |

**结论要说清楚:牛市里项目方拿到的远超 3%。** 我们不回避这一点。

### 我们据此请求的理由

1. **下行完全归零。** commission 在跌市里照抽不误;这里的项目方在仓位没有收益时收入为零,
   而那正是持有者最需要金库不被抽血的时候。
2. **与持有者严格同向,且持有者永远拿更多。** 每一笔分配里持有者拿 60%、项目方拿 40%,
   持有者恒为项目方的 **1.5 倍**。项目方无法在不同时把 1.5 倍交给持有者的前提下多拿一分。
3. **两个数都是 `constant` 且无 setter。** 部署后任何人(含项目方、含 Guardian)都无法调整,
   除非重新部署一套新 factory 并由 Flap 重新注册。
4. **税这一层不抽成,不与 Flap 的 commission 机制竞争。** `commissionReceiver` 是发币参数,
   本 factory 完全不触碰,Flap 若要对本金库的代币启用 commission,不受本设计阻碍。
