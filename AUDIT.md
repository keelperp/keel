...
**日期**：2026-08-31 · **链**：BNB Chain (56) · **状态**：factory 已上主网 `0x40dc17CC2FB7C584C87fA23d52782ea57FAC1cf1`,**未发任何代币**
...
| **006** 集成测试 | ✅ | **31 个 forge 测试 + 33 条实测断言,全绿**。forge 覆盖 schema、factory 全部守卫、beacon 归属、初始化、trigger 授权与重放、三个工作函数的 revert 路径、stipend 守卫、gas 预算;`tools/verify.py` 覆盖仓位生命周期,以原子 `eth_call` 打在主网当前状态上。两者都有断言与退出码。见下节的形态说明。 |
...
`LeverVaultFactory` 的 initcode 为 28,463 字节,在 EIP-3860 的 49,152 之内。
...
factory 已于 block 119,329,413 部署至 BNB Chain:

| | 地址 | runtime |
|---|---|---:|
| `LeverVaultFactory` | `0x40dc17CC2FB7C584C87fA23d52782ea57FAC1cf1` | 6,476 |
| `LeverBeacon` | `0xA9ff7329a4F1196819BE56e54AaDDDA3E895f284` | 785 |
| `LeverVault`(implementation) | `0x229fF752270734718c63a961C8a0f29e0AFDE3E6` | 20,257 |
...
**地址与主网相同**:`0x40dc17CC2FB7C584C87fA23d52782ea57FAC1cf1`(tx `0x40983fb2…`,block
128,473,683)。这次部署本身证明了按链分流的逻辑在真实链上生效——同样的字节码在 97 上把
`beacon.owner()` 解析成 `0x76Fa8C52…`(测试网 Guardian),在 56 上解析成 `0x9e27098d…`(主网
Guardian),都不是部署者。

**但在这条链上无法创建金库,原因不是配置而是市场本身不存在。** 实测:

| 依赖 | 测试网(97) | 结论 |
|---|---|---|
| Venus Comptroller `0x94d1820b…` | 有代码,vBNB 已上市,可借 16.3 tBNB | 可用,**CF 70%**(主网 80%) |
| vBNB `0x2E7222e5…` | 有代码 | 可用 |
| Flap TriggerService `0x560E9830…` | 有代码 | 可用 |
| WBNB | 与主网同址 | 可用 |
| **含 WBNB 的 PancakeSwap V3 池** | **不存在** | **阻断** |

最后一行是硬阻断。建仓路径**必须**闪电贷:Venus 在借款前检查抵押品,而借来的钱要等入账之后
才算抵押品,不闪一次过不去。而 BSC 测试网的 V3 实际上是空的——`getPool` 对 8 个代币 × 4 个
费率共 32 个 WBNB 组合**全部返回零地址**,且最近 40,000 个块内没有任何 `PoolCreated` 事件。
两个测试 USDT 的 `mint` 都是 `Ownable` 且 owner 是他人,所以也无法自建池。

CF 的差异反而不是问题:`_cf()` 从链上读 `markets(vBNB)`,健康度下限会把杠杆自然约束到
约 2.4x(`CF·L/(L-1) = 1.20` 在 CF 0.70 处的解),那是下限在起作用而非故障。

因此仓位生命周期改由 `tools/verify.py` 在**主网当前状态**上验证:33 条断言,每条一次原子
`eth_call`,探针字节码以 state override 注入。它覆盖的是真实的 Venus 市场与真实的 V3 池深度,
比在一条没有流动性的链上人造一个浅池更能说明主网行为。
