...
interface IVaultPortalTypes {
    ...
    event FlapTaxVaultTokenCreated(
        address indexed token, address indexed vault, address indexed vaultFactory
    );
    ...
    event AuditReportSubmitted(
        address indexed taxToken, address indexed auditor, RiskLevel riskLevel, string ipfsCid
    );
...
interface IVaultPortal is IVaultPortalTypes {
    ...
    /// @return token The address of the newly created tax token
    function newTokenV6WithVault(NewTokenV6WithVaultParams calldata params)
        external
        payable
        returns (address token);
