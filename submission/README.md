...
| | Address | |
|---|---|---|
| `LeverVaultFactory` | `0xf68e42BB99baBD2D0e2c365B438c81E4269AaC7f` | **this is the one to register** — a 279-byte `BeaconProxy`; its logic is the 7,745-byte implementation at `0x9319f9fFC7227aA7BC4c68297c1ffF0557144124`, behind the Guardian-owned `LeverFactoryBeacon` at `0xFe6747aD23A0d24998A6DF969d966AE488028DD2`. **This address does not move when the Guardian upgrades it** |
| `LeverBeacon` | `0x38424E67241c8f8C0A2575c8EBBb1DA2778843bB` | 785 bytes; owner is the Flap Guardian on each chain, transferred inside the constructor, so the deployer never held upgrade authority. The Guardian can replace the implementation behind it, which is rule 009's proxy exemption working as intended and the only way any constant in the vault changes |
| `LeverVault` (implementation) | `0x31b4037e35D356B5E09696f06b59169aB11A650F` | 23,221 bytes, sitting behind that beacon — the code that carries the exit-path floor. `MAX_SWAP_SLIP_BPS()` reads 300 at this address on both chains |
...
`0x8666262877046df9f4B338B9D7f1a30d55688A5c` (nonce 1, block 120,062,943 on 56) is the set this
...
between the retired implementation's 19,462 and the 23,221 now behind the beacon. What it buys is a
...
0x31b4037e35D356B5E09696f06b59169aB11A650F "MAX_SWAP_SLIP_BPS()(uint256)"` returns 300 on both
chains, and the same call against the retired `0xAF3A1d97…` reverts.

What we have instead of a testnet run is 106 checks, all green: 65 forge tests, 33 assertions made
against BNB Chain's current live state across seven atomic `eth_call`s, and 8 vault-UI package
checks. Plain `forge test` also exits 0 — 40 passed, 0 failed, 15 skipped (55 total tests across
18 suites) — the skips being thirteen suites (and two individual tests inside `LeverVaultAuth`) that
...
23,221 against the local build — which is the row that would have caught this package still
