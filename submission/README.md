...
| | Address | |
|---|---|---|
| `LeverVaultFactory` | `0x62D1C54CeA0a03096741dc72e51E7a4c5Ec0ACFA` | **this is the one to register** — a 279-byte `BeaconProxy`; its logic is the 7,745-byte implementation at `0x2056d0Bb15bCf98CF9ddFbEEBc899eF6BAcb0F45`, behind the Guardian-owned `LeverFactoryBeacon` at `0x3857a456D80e21467cC0EAcEE1F6D05DFab3a3b5`. **This address does not move when the Guardian upgrades it** |
| `LeverBeacon` | `0xf212DD92b89944367Ce674F638454576Cfa1B6cB` | 785 bytes; owner is the Flap Guardian on each chain, transferred inside the constructor, so the deployer never held upgrade authority. The Guardian can replace the implementation behind it, which is rule 009's proxy exemption working as intended and the only way any constant in the vault changes |
| `LeverVault` (implementation) | `0x4F6f3D2E103A90599B994623B058BF8C984Adc4B` | 22,216 bytes, sitting behind that beacon — the code that carries the exit-path floor. `MAX_SWAP_SLIP_BPS()` reads 300 at this address on both chains |
...
`0x8666262877046df9f4B338B9D7f1a30d55688A5c` (nonce 1, block 119,716,853 on 56) is the set this
...
between the retired implementation's 19,462 and the 22,216 now behind the beacon. What it buys is a
...
0x4F6f3D2E103A90599B994623B058BF8C984Adc4B "MAX_SWAP_SLIP_BPS()(uint256)"` returns 300 on both
chains, and the same call against the retired `0xAF3A1d97…` reverts.

What we have instead of a testnet run is 72 checks, all green: 47 forge tests, 33 assertions made
...
22,216 against the local build — which is the row that would have caught this package still
