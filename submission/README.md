...
| | Address | |
|---|---|---|
| `LeverVaultFactory` | `0xbfFcBB69574774EeE211E7AfdBF41187c3278607` | **this is the one to register** — a 279-byte `BeaconProxy`; its logic is the 7,745-byte implementation at `0x5b8a4E2295297cf39635f6A8b43Db4c9a8d0Cb22`, behind the Guardian-owned `LeverFactoryBeacon` at `0xb36f6F95D07f137d54c4D6224063FfC5Fb789175`. **This address does not move when the Guardian upgrades it** |
| `LeverBeacon` | `0x90b6Cba470Ba77CB1cb3d6455FB55D2681ea5b6D` | 785 bytes; owner is the Flap Guardian on each chain, transferred inside the constructor, so the deployer never held upgrade authority. The Guardian can replace the implementation behind it, which is rule 009's proxy exemption working as intended and the only way any constant in the vault changes |
| `LeverVault` (implementation) | `0x4f6f9d028DFeCD11DEF6EB8e8862dae80C4A550b` | 23,155 bytes, sitting behind that beacon — the code that carries the exit-path floor. `MAX_SWAP_SLIP_BPS()` reads 300 at this address on both chains |
...
`0x8666262877046df9f4B338B9D7f1a30d55688A5c` (nonce 1, block 120,052,827 on 56) is the set this
...
between the retired implementation's 19,462 and the 23,155 now behind the beacon. What it buys is a
...
0x4f6f9d028DFeCD11DEF6EB8e8862dae80C4A550b "MAX_SWAP_SLIP_BPS()(uint256)"` returns 300 on both
chains, and the same call against the retired `0xAF3A1d97…` reverts.

What we have instead of a testnet run is 104 checks, all green: 63 forge tests, 33 assertions made
...
14 suites) — the skips being twelve suites (and two individual tests inside `LeverVaultAuth`) that
...
23,155 against the local build — which is the row that would have caught this package still
