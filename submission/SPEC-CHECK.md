...
Deployed by `0x1544A8fCE3a3c39E0a744a13392981bEcDF014f4` in tx `0x88afc2d3bf…`, block 119,347,243.
The same bytecode from the same deployer and nonce is at the same address on chain 97 (tx
`0xba13570ab6…`, block 128,491,108), which is itself the evidence that the per-chain Guardian
...
only factory is `0x1FBa768c7E78B83edAF99c5094a8ED44A5fdF45B`; the live schema text is at
...
| **006** Integration tests | PASS, with the gap stated below | 41 forge tests + 33 live-state assertions + 8 vault-UI checks = **82, all green**. Plain `forge test` also exits 0: 36 passed, 0 failed, 5 skipped. The position-lifecycle suite is the skip — see *Checked but not exercised* |
...
`LeverVaultFactory` 7,145, `LeverBeacon` 785, and the registered `BeaconProxy` 279. The
factory's initcode is 7,192 bytes, inside EIP-3860's 49,152. It no longer carries the vault's
