...
Deployed by `0x1544A8fCE3a3c39E0a744a13392981bEcDF014f4` in tx `0x88afc2d3bf…`, block 119,329,413.
The same bytecode from the same deployer and nonce is at the same address on chain 97 (tx
`0xba13570ab6…`, block 128,473,683), which is itself the evidence that the per-chain Guardian
...
only factory is `0x40dc17CC2FB7C584C87fA23d52782ea57FAC1cf1`; the live schema text is at
...
| **006** Integration tests | PASS, with the gap stated below | 31 forge tests + 33 live-state assertions + 8 vault-UI checks = **69, all green**. Plain `forge test` also exits 0: 29 passed, 1 skipped. The position-lifecycle suite is the skip — see *Checked but not exercised* |
...
Every deployable contract is inside EIP-170: `LeverVault` 20,257 (4,994 of margin),
`LeverVaultFactory` 6,476, `LeverBeacon` 785. The factory's initcode is 28,463 bytes — it carries
...
**The position-lifecycle forge suite needs an archive RPC and is skipped by default.**
`test/LeverVaultPosition.t.sol` — `test_taxAccumulatesThenBuildsToTarget`,
...
These paths take about 50 seconds of wall time across multi-step Venus and PancakeSwap state, BSC
