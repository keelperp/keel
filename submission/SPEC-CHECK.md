...
Deployed by `0x1544A8fCE3a3c39E0a744a13392981bEcDF014f4` in tx `0xd99db992…`, block 120,046,230.
The same bytecode from the same deployer and nonce is at the same address on chain 97 (tx
`0x1e5fc449…`, block 129,190,762), which is itself the evidence that the per-chain Guardian
...
only factory is `0x487Bd18860c321b6Fa01e9F95B3F9BF878c4939B`; the live schema text is at
...
| **006** Integration tests | PASS, with the gap stated below | 61 forge tests + 33 live-state assertions + 8 vault-UI checks = **102, all green**. Plain `forge test` also exits 0: 40 passed, 0 failed, 11 skipped (51 total). Eleven archive-only suites plus two tests inside `LeverVaultAuth` are the skips — see *Checked but not exercised* |
