...
The four rows marked **(fork)** live in `test/LeverVaultPosition.t.sol`, one of thirteen suites (plus
...
the full list and why. That is why plain `forge test` reports **40 passed, 0 failed, 15 skipped**
(55 total tests) — every skip there is an archive-only suite declining to run, not a failure. The
full run used for this submission is 65 forge tests + 33 live-state assertions + 8 vault-UI
checks = **106, all green**.
...
Those addresses are in [`FACTORY.md`](FACTORY.md) and `deployments/56.json`; the factory to register is `0xf68e42BB99baBD2D0e2c365B438c81E4269AaC7f`.
