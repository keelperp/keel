...
Register **`0x2559DD277E5a8E2f6d8594Deded3eD1025e6402C`**.
...
|---|---|---:|---|
| `LeverVaultFactory` — **this is the one to register** | `0x2559DD277E5a8E2f6d8594Deded3eD1025e6402C` | 279 | a `BeaconProxy`. 279 bytes because it is only the proxy; the logic is at the implementation below. **This address never moves across upgrades** |
| `LeverFactoryBeacon` | `0x4EB9bE3Ec27673c5A7B88e1420fA349d68A69910` | 785 | Guardian-owned. `upgradeTo` here replaces the factory's code |
| `LeverVaultFactory` (implementation) | `0x04F0AAc92361f66E0B78c8f6d03656Ab6A9a460d` | 7,745 | the factory's actual logic, sitting behind that beacon |
| `LeverBeacon` | `0x16Bb8430D443d120BC4bDe71d43d2A9EeA759E0B` | 785 | Guardian-owned. `upgradeTo` here replaces the code in **every** live vault at once |
| `LeverVault` (implementation) | `0xC57c9D2ac2459e814Bc93C885C8D9F9E6d6Cd6A1` | 22,678 | the vault logic every `BeaconProxy` vault runs |
...
|---|---:|---|---|
| BNB Chain, 56 | 119,738,718 | `0xd6330a771e797a73f39f8306a6003beb20393ece822b6251d893362eb920a9e3` | `0x9e27098dcD8844bcc6287a557E0b4D09C86B8a4b` — Flap Guardian, BNB Chain |
| BSC testnet, 97 | 128,883,139 | `0x3ec5cef590240793dcb6000ab9ebf95c93b14a2b01aee9aec047511c6ddd0bd0` | `0x76Fa8C526f8Bc27ba6958B76DeEf92a0dbE46950` — Flap Guardian, testnet |
...
```solidity
registerVaultFactory(0x2559DD277E5a8E2f6d8594Deded3eD1025e6402C, /* enabled */ true, /* official */ false, riskLevel)
```
...
- **The address to register never moves.** `upgradeTo` on `LeverFactoryBeacon` replaces the
  factory's code while `0x2559DD277E5a8E2f6d8594Deded3eD1025e6402C` stays exactly where it is.
...
deployable contract here is inside it — the largest is `LeverVault` at 22,678. EIP-3860 caps
...
F=0x2559DD277E5a8E2f6d8594Deded3eD1025e6402C   # the proxy — register this
FB=0x4EB9bE3Ec27673c5A7B88e1420fA349d68A69910  # factory beacon, Guardian-owned
FI=0x04F0AAc92361f66E0B78c8f6d03656Ab6A9a460d  # factory implementation
B=0x16Bb8430D443d120BC4bDe71d43d2A9EeA759E0B   # vault beacon, Guardian-owned
VI=0xC57c9D2ac2459e814Bc93C885C8D9F9E6d6Cd6A1  # vault implementation

# runtime sizes: 279 / 785 / 7,745 / 785 / 22,678
...
cast tx 0xd6330a771e797a73f39f8306a6003beb20393ece822b6251d893362eb920a9e3 blockNumber --rpc-url $R
cast tx 0x3ec5cef590240793dcb6000ab9ebf95c93b14a2b01aee9aec047511c6ddd0bd0 blockNumber --rpc-url $T
...
bash scripts/test.sh       # 52 forge tests + 33 live-state assertions + 8 vault-UI checks = 69
```
